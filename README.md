...
Порядок зон: `a`, `b`, `d`. На зону: 2 минуты pod-kill (каждые 30 с), 2 минуты покой, 2 минуты loss 30% (`direction: both`), покой, 2 минуты delay 500 мс, покой, 2 минуты изоляция, restore, покой. После каждого шага скрипт печатает счётчики и `_count`. Цифры результатов — плейсхолдеры до прогона.
