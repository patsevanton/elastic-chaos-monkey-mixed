...
Исключение из инфрах-правила HDD: **PVC Elasticsearch — `yc-network-ssd`**. Ноды k8s, Rally VM и Headscale VM — HDD.
...
- На VM: сервер Headscale **0.29.3** (DEB) + Tailscale-клиент ≥ **1.80.0** как subnet router на `10.0.1.0/24`–`10.0.4.0/24`.
...
- Отдельная VM в **`ru-central1-e`**: **8 vCPU / 16 ГБ**, HDD **100 ГиБ**, **без публичного IP** (`nat = false`).
...
3. **Зона `b`** — `yc compute instance stop` единственной ноды node group `b`, держать выключенной, затем `start`. Grafana/Traefik/exporter/ECK/Kibana живы в `a` и `d`. Headscale VM в `a` не стопаем.
...
- Зона `b`: только `yc compute instance start` той же VM, не `terraform apply`, не удаление node group. VPN не падает (Headscale в `a`).
